        ...
        public void change_order_status(int orderid, int status, string user_token)
        {
            try
            {
                ...
                if (File.Exists(path))
                {
                    ...
                    readText = readText.Replace("@Url", GetBaseUrl() + "/order?Id=" + record.Id);
                ...
                foreach (var data in query)
                {
                    ...
                    if (File.Exists(path))
                    {
                        ...
                        readText = readText.Replace("@Email", data.Email);
                        readText = readText.Replace("#Number", "#" + record.Id);
                        readText = readText.Replace("@Url", GetBaseUrl() + "/admin/order?Id=" + record.Id);
            ...
            try
            {
                ...
                if (result.ToUpper() == "CAPTURED" || result.ToUpper() == "SUCCESS" || result.ToUpper() == "AUTHORISED")
                {
                    ...
                    if (File.Exists(path))
                    {
                        ...
                        readText = readText.Replace("@Url", GetBaseUrl() + "/order?Id=" + orderid);
