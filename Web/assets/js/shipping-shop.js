...
function GetProductsList() {
    ...
    if (!(allParams.sort == "-1" && (location.href == "/shipping-shop" || location.href == "/ar/shipping-shop"))) {
        apiURL = apiURL + "&sort=" + (allParams.sort ? allParams.sort : "-1").toString();
        apiURL = apiURL.replace("?&", "?");
        history.pushState({}, null, apiURL);
    }

    if (!(allParams.start == "1" && (location.href == "/shipping-shop" || location.href == "/ar/shipping-shop"))) {
        apiURL = apiURL + "&start=" + (allParams.start ? allParams.start : "-1").toString();
        apiURL = apiURL.replace("?&", "?");
        history.pushState({}, null, apiURL);
    }

    if (!(allParams.end == "50" && (location.href == "/shipping-shop" || location.href == "/ar/shipping-shop"))) {
...
function FillProducts(data, end, start) {
    if (data.length > 0) {
        ...
        for (index; index < data.length; index++) {
            ...
            items.push(`<div class="product product-list" style="border-bottom: solid 1px #eee;margin-bottom: 20px;">
                                    ...
                                    <div class="product-details" style="width:100%;min-height: 265px;">
                                        ...
                                        <div class="product-price"><span style="font-size: 12px;font-weight: 200;">${item.Usd > 0 ? (IsArabic ? "بدءا من" : "Starting") : ""} </span> ${item.Usd > 0 ? Math.ceil((item.Usd * rate_value).toFixed(2)) : (IsArabic ? "قريباً" : "Coming Soon")} ${item.Usd > 0 ? rate_code : ""}</div>
                                        <div class="ratings-container">
                                        ...
                                        <div class="product-action" style="position: absolute;">
                                            <a href="${productURL}" class="btn-product" title="${IsArabic ? "أطلب الأن" : "Order Now"}" style="min-width: 250px;">${IsArabic ? "أطلب الأن" : "Order Now"} </a>
                                        </div>
                                    </div>
                                </div>`);
        }
        ...
        $('.showing-info').html(' ');
        if (IsArabic)
            $('.list-Noresult').html('<div style="font-weight: 500;color: #F29811;padding: 5px;font-size: 20px;"><span class="w-icon-search-plus" style="padding-right: 5px;padding-left: 5px;"></span>لم يتم العثور على نتائج!</div>');
        else
            $('.list-Noresult').html('<div style="font-weight: 500;color: #F29811;padding: 5px;font-size: 20px;"><span class="w-icon-search-plus" style="padding-right: 5px;padding-left: 5px;"></span>No result found!</div>');
        $('.list').html(' ');
        $('body , html').animate({
                            ...
                            <div class="accordion accordion-bg accordion-gutter-md accordion-border show-code-action">
                                <div class="card">
                                    <div class="card-header">
                                        <a id="cbCategory${item.Id}" href="${item.IsHasSubCategory ? "#collapse" : "#"}"  onclick="CheckedGategory(${item.Id});" style="background: transparent;font-size: 14px;font-weight: 100;padding: 1rem 0 1rem 0.2rem;">${IsArabic ? item.NameAr : item.NameEn}</a>
                                    </div>
                                </div>
                            </div>
                `);
...
function CleanAll() {
    if (IsArabic)
        window.location.href = "/ar/shipping-shop.aspx";
    else
        window.location.href = "/shipping-shop.aspx";
}
